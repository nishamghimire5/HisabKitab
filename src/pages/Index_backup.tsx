...
const Index = () => {
  ...
  const { user } = useAuth();
  const navigate = useNavigate();// Load trips from Supabase
  useEffect(() => {
    ...
    try {
      ...
      if (newTrip.initialFriends && newTrip.initialFriends.length > 0) {
        try {
          ...
          toast.success(`Trip created! Invitations sent to ${newTrip.initialFriends.length} friend(s).`);        } catch (inviteError) {
